...
/**
 * {@link VisualiserView} is responsible for setting up and drawing the visualization of the music.
 */
public class VisualiserView extends View {
    ...
    public void updateFFT(byte[] bytes) {
        ...
        float trebleTotal = 0;
        for (int i = 0; i < bytes.length * MID_SEGMENT_SIZE; i++) {
    ...
    /**
     * Sets the visibility of the bass circle, mid-range square, the treble triangle
     *
     * @param showBass boolean determining if bass circle, mid-range square, the treble triangle should be shown
     */
